...
function ConvLSTM:buildHidden(ConvNN)
	local hidden = nn.Sequential()
	-- input is {input, output(t-1), cell(t-1)}, but we only need {input, output(t-1)}
	hidden:add(nn.NarrowTable(1,2))
	local input2hidden = ConvNN or nn.Linear(self.inputSize, self.outputSize)
...
function ConvLSTM:buildCell(ConvNN)
	-- build
	self.inputGate = self:buildInputGate() 
	self.forgetGate = self:buildForgetGate()
	self.hiddenLayer = self:buildHidden(ConvNN)
...
function ConvLSTM:buildModel(ConvNN)
	-- build components
	self.cellLayer = self:buildCell(ConvNN)
